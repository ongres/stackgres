# :rocket: Release 1.18.9 (2026-09-01)

## :notepad_spiral: NOTES

StackGres 1.18.9 is out! :confetti_ball: :champagne: 

> This patch release includes a backport to run postgres_exporter as a low-privilege `monitor` role and route its `dblink` calls through `SECURITY DEFINER` connect functions (#3211)

So, what you are waiting for to try this release and have a look to the future of StackGres! 

## :sparkles: NEW FEATURES AND CHANGES

* Backport to run postgres_exporter as a low-privilege `monitor` role and route its `dblink` calls through `SECURITY DEFINER` connect functions (#3211)
