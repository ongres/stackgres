...
1. [ ] Make sure all tasks marked with label ~"target_version::1.14.1" are done.

# Release steps

1. [ ] Create local branch `release-1.14.1` from `main-1.14`:
    ```
    git checkout "main-1.14" && git pull && git checkout -b "release-1.14.1"
    ```
1. [ ] Update project version to `1.14.1`:
    ```
    sh -x stackgres-k8s/ci/utils/update-version.sh "1.14.1"
    ```
1. [ ] Update `CHANGELOG.md` (review commit messages to populate the changelog: `git log`)
1. [ ] Add 1.14.1 section in `doc/content/en/01-introduction/06-versions/_index.md` with values from `stackgres-k8s/src/common/src/main/resources/versions.properties`
1. [ ] Check the changes to ensure everything is correct before commit:
    ```
    git diff
    ```
1. [ ] Commit changes with message `version: 1.14.1`:
    ```
    git commit -S -a -m "version: 1.14.1"
    ```
1. [ ] Push `release-1.14.1` branch:
     ...
     **As an alternative approach [here](https://gitlab.com/snippets/1985684) is a handy snippet that allows to spin up such an instance in AWS.**
     ...
     Now we can push `release-1.14.1` branch and wait for the pipeline to complete:
    ```
    git push origin "release-1.14.1" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    ```
...
1. [ ] Perform preflight for operator images (make each of them pass pass the index health check by opening each image project):
    ...
    STACKGRES_VERSION="1.14.1" IMAGE_TAG="$(git rev-parse --short=8 HEAD)" sh get-images.sh | grep -F quay.io/stackgres/ | sed 's#quay\.io/stackgres/#registry.gitlab.com/ongresinc/stackgres/stackgres/#' | xargs -I % sh preflight.sh %
    ```
...
1. [ ] Create tag `1.14.1`:
    ...
    git tag "1.14.1"
    ```
1. [ ] Push tag `1.14.1` to the origin and wait for the pipeline to complete:
    ```
    git push origin "1.14.1"
    ```
...
1. [ ] Perform preflight for operator images (publish each of them by opening each image project):
    ...
    ```
    cd stackgres-k8s/install/operator-sdk/openshift-certification/
    STACKGRES_VERSION="1.14.1" IMAGE_TAG="1.14.1" sh get-images.sh | grep -F quay.io/stackgres/ | xargs -I % sh preflight.sh %
    ```
1. [ ] Create PR on Red Hat Certified Operators (wait for its completion):
    ...
    > 
    > ```
    > git checkout -b "fix-bundle-1.14.1"
    > git add .
    > git commit -m "fix: operator bundle deployment"
    > git push origin "fix-bundle-1.14.1" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    > ```
    > 
    > Repeat the PR creation step above.
    > 
    > When PR is merged create a MR to `main-1.14.1` branch.
...
1. [ ] Create PR on OperatorHub (wait for its completion):
    ...
    > 
    > When PR is merged create a MR to `main-1.14.1` branch.
1. [ ] Edit the [release notes of tag 1.14.1](https://gitlab.com/ongresinc/stackgres/-/releases/new?tag_name=1.14.1) by Copying and Pasting `CHANGELOG.md` section for version `1.14.1` (GitLab)
1. [ ] Merge local branch `release-1.14.1` into `main-1.14`:
    ```
    git checkout "main-1.14" && git pull && git merge --ff-only "release-1.14.1"
    ```
1. [ ] Update version to be `1.14.2-SNAPSHOT`:
    ```
    sh -x stackgres-k8s/ci/utils/update-version.sh "1.14.2-SNAPSHOT" "main-1.14"
    git commit -a -m "version: 1.14.2-SNAPSHOT"
    git push
    ```
1. [ ] Create branch `merge-1.14.1` from `main`:
    ```
    git checkout main && git pull && git checkout -b "merge-1.14.1"
    ```
1. [ ] Merge branch `main-1.14` into `merge-1.14.1`:
    ```
    git merge "main-1.14"
    ```
1. [ ] Push `merge-1.14.1` to origin, create the merge request to merge it into `main` and wait for the pipeline to complete fixing any encountered issues:
    ```
    git push origin "merge-1.14.1"
    ```
...
1. [ ] Edit `.gitlab-ci.yml`:
  * Change `STACKGRES_FULL_VERSIONS` by setting `1.14.1` as the first value.
1. [ ] Commit changes with message `version: 1.14.1`:
    ```
    git commit -a -m 'version: 1.14.1'
    ```
...
~~~
# :rocket: Release 1.14.1 (${DATE})

## :notepad_spiral: NOTES

StackGres 1.14.1 is out! :confetti_ball: :champagne: 
...
/label ~StackGres ~"target_version::1.14.1" ~"team::DEV" 
/milestone %"StackGres 1.14.1"
/confidential 
