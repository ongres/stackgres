...
```
Release StackGres 1.16.0-beta1
```
...
1. [ ] Make sure all tasks marked with label ~"target_version::1.16.0-beta1" are done.

# Release steps

1. [ ] Create local branch `release-1.16.0-beta1` from `main-1.16`:
    ```
    git checkout "main-1.16" && git pull && git checkout -b "release-1.16.0-beta1"
    ```
1. [ ] Update project version to `1.16.0-beta1`:
    ```
...
1. [ ] Push `release-1.16.0-beta1` branch:
     ...
     **As an alternative approach [here](https://gitlab.com/snippets/1985684) is a handy snippet that allows to spin up such an instance in AWS.**
     ...
     Now we can push `release-1.16.0-beta1` branch and wait for the pipeline to complete:
    ```
    git push origin "release-1.16.0-beta1" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    ```
...
1. [ ] Create PR on Red Hat Certified Operators (wait for its completion):
    ...
    > 
    > ```
    > git checkout -b "fix-bundle-1.16.0-beta1"
    > git add .
    > git commit -m "fix: operator bundle deployment"
    > git push origin "fix-bundle-1.16.0-beta1" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    > ```
    > 
...
1. [ ] Merge local branch `release-1.16.0-beta1` into `main-1.16`:
    ```
    git checkout "main-1.16" && git pull && git merge --ff-only "release-1.16.0-beta1"
    ```
1. [ ] Update version to be `1.16.0-SNAPSHOT`:
    ...
    git push
    ```
1. [ ] Create branch `merge-1.16.0-beta1` from `main`:
    ```
    git checkout main && git pull && git checkout -b "merge-1.16.0-beta1"
    ```
...
1. [ ] Push master to origin:
    ...
    git push
    ```
1. [ ] Push tag `1.16.0-beta1` to origin:
    ```
    git push origin 1.16.0-beta1
    ```
...
~~~
# :rocket: Release 1.16.0-beta1 (${DATE})

## :notepad_spiral: NOTES

StackGres 1.16.0-beta1 is out! :confetti_ball: :champagne: 
...
## :twisted_rightwards_arrows: [FULL LIST OF COMMITS](https://gitlab.com/ongresinc/stackgres/-/commits/1.16.0-beta1)
~~~

/label ~StackGres ~"target_version::1.16.0-beta1" ~"team::DEV" 
/milestone %"StackGres 1.16.0-beta1"
/confidential 
