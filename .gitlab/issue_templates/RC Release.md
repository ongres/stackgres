...
1. [ ] Make sure all tasks marked with label ~"target_version::1.14.0-rc3" are done.

# Release steps

1. [ ] Create local branch `release-1.14.0-rc3` from `main-1.14`:
    ```
    git checkout "main-1.14" && git pull && git checkout -b "release-1.14.0-rc3"
    ```
1. [ ] Update project version to `1.14.0-rc3`:
    ```
...
1. [ ] Push `release-1.14.0-rc3` branch:
     ...
     **As an alternative approach [here](https://gitlab.com/snippets/1985684) is a handy snippet that allows to spin up such an instance in AWS.**
     ...
     Now we can push `release-1.14.0-rc3` branch and wait for the pipeline to complete:
    ```
    git push origin "release-1.14.0-rc3" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    ```
...
1. [ ] Create PR on Red Hat Certified Operators (wait for its completion):
    ...
    > 
    > ```
    > git checkout -b "fix-bundle-1.14.0-rc3"
    > git add .
    > git commit -m "fix: operator bundle deployment"
    > git push origin "fix-bundle-1.14.0-rc3" -o ci.variable="DO_IMAGES=true" -o ci.variable="DO_NATIVE=true" -o ci.variable="DO_ARM=true" -o ci.variable="DO_RELEASE_IMAGE=true"
    > ```
    > 
...
1. [ ] Merge local branch `release-1.14.0-rc3` into `main-1.14`:
    ```
    git checkout "main-1.14" && git pull && git merge --ff-only "release-1.14.0-rc3"
    ```
1. [ ] Update version to be `1.14.0-SNAPSHOT`:
    ...
    git push
    ```
1. [ ] Create branch `merge-1.14.0-rc3` from `main`:
    ```
    git checkout main && git pull && git checkout -b "merge-1.14.0-rc3"
    ```
...
1. [ ] Push master to origin:
    ...
    git push
    ```
1. [ ] Push tag `1.14.0-rc3` to origin:
    ```
    git push origin 1.14.0-rc3
    ```
...
`helm upgrade -n "stackgres" "stackgres-operator" https://stackgres.io/downloads/stackgres-k8s/stackgres/1.14.0-rc3/helm/stackgres-operator.tgz`
...
/label ~StackGres ~"target_version::1.14.0-rc3" ~"team::DEV" 
/milestone %"StackGres 1.14.0-rc3"
/confidential 
