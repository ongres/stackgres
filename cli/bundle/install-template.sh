...
setup_env() {
    ...
    SERVICE_MATRIARCH=${SYSTEM_NAME}-matriarch.service
    ...
    FILE_MATRIARCH_SERVICE=${SYSTEMD_DIR}/${SERVICE_MATRIARCH}
...
_check_http_status() {
    case "$1" in
        ...
        404)
            fatal "Not found: $2
The requested StackGres version '${STACKGRES_VERSION}' does not exist on the server.
...
[ "${HAS_SYSTEMD}" = true ] && [ -s "${SYSTEMD_DIR}/${SERVICE_MATRIARCH}" ] && systemctl stop ${SYSTEM_NAME}-matriarch.service
...
if command -v systemctl; then
    systemctl disable ${SYSTEM_NAME}-containerd ${SYSTEM_NAME}-slony ${SYSTEM_NAME}-matriarch
    systemctl reset-failed ${SYSTEM_NAME}-containerd ${SYSTEM_NAME}-slony ${SYSTEM_NAME}-matriarch
    systemctl daemon-reload
fi

rm -f ${FILE_MATRIARCH_SERVICE}
...
systemd_disable() {
    $SUDO systemctl disable ${SYSTEM_NAME}-containerd ${SYSTEM_NAME}-slony ${SYSTEM_NAME}-matriarch >/dev/null 2>&1 || true
    $SUDO rm -f /etc/systemd/system/${SERVICE_CONTAINERD} /etc/systemd/system/${SERVICE_SLONY} /etc/systemd/system/${SERVICE_MATRIARCH} || true
}
...
create_systemd_service_files() {
    info "systemd: Creating service files ${SYSTEMD_DIR}/{${SERVICE_MATRIARCH},${SERVICE_SLONY},${SERVICE_CONTAINERD}}"

    $SUDO tee ${FILE_MATRIARCH_SERVICE} >/dev/null << EOF
[Unit]
Description=StackGres Matriarch
...
Restart=always
RestartSec=5s
ExecStart=${STACKGRES_DIR}/bin/matriarch
EOF
...
systemd_enable() {
    ...
    $SUDO systemctl enable ${SERVICE_CONTAINERD} ${SERVICE_MATRIARCH} ${SERVICE_SLONY} &>/dev/null
}

systemd_start() {
    info "systemd: Starting ${SYSTEM_NAME} unit"
    $SUDO systemctl start ${SERVICE_CONTAINERD} &>/dev/null
    $SUDO systemctl start ${SERVICE_MATRIARCH} &>/dev/null
