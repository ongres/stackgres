+++
title = "How to Run Metis on Top Of StackGres"
date = 2024-02-12T14:00:00+01:00
author = ["afurmanek"]
tags = ["postgres", "kubernetes", "metis"]
planet = "aht"
draft = false
thumbnail = "img/blog/how-to-run-metis-on-top-of-stackgres.jpg"
description = "In this how-to guide, we'll walk you through the whole process of setting up Metis on a Kubernetes cluster using StackGres."
+++

# How To Run Metis On Top Of StackGres
In this how-to guide, we'll walk you through the whole process of setting up [Metis](https://www.metisdata.io/) on a Kubernetes cluster using [StackGres](https://stackgres.io/).

Metis is a comprehensive database observability tool. It can track your live database activity, suggest improvements for schema, indexes, queries, configuration, and more. By onboarding your PostgreSQL server with Metis, you get access to live infrastructural metrics (CPU, memory usage) and database metrics (transactions, buffers, caches). Metis can look for anomalies in performance, index usage, or data distribution. See [the documentation on how to monitor & troubleshoot your databases](https://docs.metisdata.io/Monitor%20and%20troubleshoot%20databases).

This guide is also available as a [video](https://www.youtube.com/watch?v=neB0JH7hfjE)
...
With support for [more than 150 Postgres extensions](https://stackgres.io/extensions), StackGres is the most extensible Postgres platform available. It also provides support for [Babelfish for Postgres](https://babelfishpg.org/) (which brings SQL Server compatibility, at the wire protocol level, SQL and T-SQL) and integrations with Citus for sharding Postgres, Timescale for time-series, Supabase and now, FerretDB.
...
![Metis Deploy](/img/blog/metis-deploy.png "Deploy")

Select **_Postgres_** and click **_Next_**:

![Metis Select Vendor](/img/blog/metis-select-vendor.png "Select vendor")

Metis instructs us how to configure the user and grant necessary permissions. We already did that. Click **_Next_**.

![Metis Host Connection](/img/blog/metis-host-connection.png "Metis Host Connection")

Metis asks for the connection string now:

![Metis Configuration](/img/blog/metis-configuration.png "Metis Configuration")
...
![Metis Connection String](/img/blog/metis-connection-string.png "Metis Connection String")

Click **_Next_**. Finally, click on **_Helm_** and copy the script to deploy Metis.

![Metis Helm](/img/blog/metis-helm.png "Metis Helm")
...
![Metis Monitoring](/img/blog/metis-monitoring.png "Metis Monitoring")

When you click on the host, you should see the properties and databases:

![Metis Database](/img/blog/metis-databases.png "Metis Database")

You can also click on database **_metis_** and get all the details:

![Metis Observability](/img/blog/metis-observability.png "Metis Observability")
...
![Metis Table Sizes](/img/blog/metis-table-sizes.png "Metis Table Sizes")

You can also go to **_Top Queries_** widget and notice that new queries are flowing to the database:

![Metis Top Queries](/img/blog/metis-top-queries.png "Metis Top Queries")
...
Here's the [Stackgres runbook](https://stackgres.io/doc/latest/runbooks/) and [Metis Guide](https://docs.metisdata.io/SetupMetis/) to get you started.
