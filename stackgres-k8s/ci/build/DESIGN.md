...
| `container_engine_socket_volume` | Whatever starts a container from inside a build container needs the engine of the host. The socket is mounted at the path docker uses with both engines, so nothing inside has to know which one is running. podman is daemonless and only has a socket while `podman system service` is serving one, so `container_engine_serve_socket` starts one for the duration of the build container when none is already there, which is what makes it work in a job pod, where enabling the systemd user unit is not an option. |
| `container_engine_testcontainers_env` | Testcontainers starts its containers on the engine of the host, so they are siblings of the build container. It is told the path the socket has outside through `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, and to reach the ports those containers publish through `TESTCONTAINERS_HOST_OVERRIDE=host.containers.internal`, the name podman resolves to the host in every container it runs. Without it the containers start and the tests time out connecting to them on their own loopback. The reaper would need a privileged container with a rootless podman, so it is disabled instead. This only holds where a container can have a network of its own: in a job pod they all share the network namespace of the pod, podman discards the port mappings and Testcontainers waits forever for a published port, which is why the Dev Services of Quarkus are disabled for the tests, see `%test.quarkus.datasource.devservices.enabled` in cluster-controller and stream. |
