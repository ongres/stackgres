...
* `DOCIR_REPOSITORY_URL`: Allow to set a different URL for the StackGres images registry REST API (docir) used by the SGClusters with `spec.configurations.registry.enabled` (default: `https://sgcr.dev`).
* `DOCIR_CACHE_ENABLED`: When `true` the operator is installed with the cache of the docir REST API of the Helm chart (`repository.cache.enabled`, the `stackgres-operator-docir-cache` StatefulSet) backed by the host path `DOCIR_CACHE_NODE_PATH` of the k8s nodes (`/containerd-cache/docir`, that is `DOCIR_CACHE_HOST_PATH`, `K8S_CACHE_HOST_PATH/docir` by default, on the host for kind) where the catalog is also stored by the tests, so that the tests do not reach the remote REST API on every run. Only the image requests of the Postgres versions required by the tests that exist in the catalog are preloaded (default: the value of `EXTENSIONS_CACHE_ENABLED`, `true` for the `e2e` CLI).
* `E2E_DOCIR_CACHE_PULL_IMAGES`: When `false` the cache does not copy the preloaded images into its local image registry (default: `true`).
* `E2E_REGISTRY_ENABLED`: When `true` (the default) the versions of the components (Postgres flavors and addons) and of the extensions used by the tests are derived from the docir REST API catalog instead of the `versions-*.properties` and the extensions repository index.
* `E2E_USE_PUBLISHED_IMAGES`: When `false` the unpublished images of the docir REST API are also used (default: `true`).
