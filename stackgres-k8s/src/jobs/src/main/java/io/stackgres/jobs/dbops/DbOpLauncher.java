...
@ApplicationScoped
public class DbOpLauncher {

  private static final Logger LOGGER = LoggerFactory.getLogger(DbOpLauncher.class);
  ...
  public void launchDbOp(String dbOpName, String namespace) {
    ...
    if (jobImpl.isResolvable()) {
      ...
      try {
        ...
        LOGGER.info("Unexpected exception for SgDbOp {}", dbOpName, ex);
  ...
  private List<Condition> getStartingConditions() {
  ...
  private List<Condition> getCompletedConditions() {
    final List<Condition> conditions = List.of(
  ...
  private List<Condition> getFailedConditions() {
    final List<Condition> conditions = List.of(
        DbOpsStatusCondition.DB_OPS_FALSE_RUNNING.getCondition(),
        DbOpsStatusCondition.DB_OPS_FALSE_COMPLETED.getCondition(),
  ...
  private List<Condition> getTimeoutConditions() {
    final List<Condition> conditions = List.of(
        DbOpsStatusCondition.DB_OPS_FALSE_RUNNING.getCondition(),
        DbOpsStatusCondition.DB_OPS_FALSE_COMPLETED.getCondition(),
