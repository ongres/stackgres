...
public enum ClusterPathV1 implements EnvVarPathSource<StackGresCluster> {
  ...
  PATRONI_BIN_PATH(USR_BIN_PATH, "patroni"),
  PATRONICTL_BIN_PATH(USR_BIN_PATH, "patronictl"),
  WALG_BIN_PATH(LOCAL_BIN_PATH, "wal-g"),
  HDRHISTOGRAM_BIN_PATH(LOCAL_BIN_PATH, "dump_hdrh"),
  PGBOUNCER_BIN_PATH(LOCAL_BIN_PATH, "pgbouncer"),
  POSTGRES_EXPORTER_BIN_PATH(LOCAL_BIN_PATH, "postgres_exporter"),
  FLUENTD_BIN_PATH(LOCAL_BIN_PATH, "fluentd"),
  FLUENT_BIT_BIN_PATH(LOCAL_BIN_PATH, "fluent-bit"),
  OTEL_COLLECTOR_BIN_PATH(LOCAL_BIN_PATH, "otelcol-contrib"),
  ...
  ClusterPathV1(String path) {
    this.path = path;
  }

  ClusterPathV1(String... paths) {
    this(String.join("/", paths));
  }

  ClusterPathV1(ClusterPathV1 parent, String... paths) {
