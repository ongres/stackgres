...
public class LegacyExtensionManagerTest {
  ...
  void setUp() throws Exception {
    ...
    extensionManager = new LegacyExtensionManager(extensionMetadataManager,
  ...
  void testDownloadAndExtractExtension() throws Exception {
    ...
    verify(fileSystemHandler, times(2)).createDirectories(
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))));
  ...
  void testVerifyExtension() throws Exception {
    ...
    when(fileSystemHandler
        .newInputStream(
            eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
                .resolve(extensionPackageName + LegacyExtensionManager.SHA256_SUFFIX))))
                    .then(invocation -> getClass().getResourceAsStream("/test.tgz.sha256"));
    when(fileSystemHandler
        .newInputStream(
            eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
                .resolve(extensionPackageName + LegacyExtensionManager.TGZ_SUFFIX))))
  ...
  void testInstallExtension() throws Exception {
        ...
        .newInputStream(
            eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
                .resolve(extensionPackageName + LegacyExtensionManager.TGZ_SUFFIX))))
                    .then(invocation -> getClass().getResourceAsStream("/test.tgz"));
    when(fileSystemHandler
        .list(eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_LIB_PATH.path(context(cluster))))))
            .thenReturn(Stream
                .of(Paths.get(ClusterPathV1.PG_EXTENSIONS_LIB_PATH.path(context(cluster)))
                    .resolve("test.so")));
    ...
    verify(fileSystemHandler, times(1)).createOrReplaceFile(
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
            .resolve(extensionPackageName + LegacyExtensionManager.INSTALLED_SUFFIX)));
    verify(fileSystemHandler, times(1)).createOrReplaceFile(
        eq(Paths.get(ClusterPathV1.PG_RELOCATED_LIB_PATH.path(context(cluster)))
            .resolve(extensionPackageName + LegacyExtensionManager.LINKS_CREATED_SUFFIX)));
    ...
    verify(fileSystemHandler, times(4)).createDirectories(
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_LIB64_PATH.path(context(cluster)))));
    verify(fileSystemHandler, times(1)).createDirectories(
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
            .resolve("usr/share")));
    verify(fileSystemHandler, times(1)).createDirectories(
    ...
    verify(fileSystemHandler, times(1)).createOrReplaceSymbolicLink(
        eq(Paths.get(ClusterPathV1.PG_RELOCATED_LIB_PATH.path(context(cluster)))
            .resolve("test.so")),
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_LIB_PATH.path(context(cluster)))
            .resolve("test.so")));
    ...
    verify(fileSystemHandler, times(1)).deleteIfExists(
        eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
            .resolve(extensionPackageName + LegacyExtensionManager.PENDING_SUFFIX)));
    verify(fileSystemHandler, times(0)).identical(any(), any());
  }
        ...
        .newInputStream(
            eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
                .resolve(extensionPackageName + LegacyExtensionManager.TGZ_SUFFIX))))
                    .then(invocation -> getClass().getResourceAsStream("/test.tgz"));
    when(fileSystemHandler
        .exists(eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
            .resolve("usr/lib/postgresql/12/lib").resolve("test.so"))))
                .thenReturn(false);
    StackGresClusterInstalledExtension extension = getInstalledExtension();
    ...
    when(fileSystemHandler
        .exists(eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
            .resolve("usr/lib/postgresql/12/lib").resolve("test.so"))))
                .thenReturn(true);
    when(fileSystemHandler
        .identical(eq(Paths.get(ClusterPathV1.PG_EXTENSIONS_PATH.path(context(cluster)))
