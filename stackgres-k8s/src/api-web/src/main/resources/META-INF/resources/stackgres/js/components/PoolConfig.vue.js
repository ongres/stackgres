			...
			<div class="content">
				<table class="poolConfig">
					...
					<tbody>
						<tr class="no-results">
							<td :colspan="3">
								No configurations have been found, would you like to <router-link to="/crd/create/connectionpooling/" title="Add New Connection Pooling Configuration">create a new one?</router-link>
							</td>
						</tr>
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpoolconfig-'+conf.data.metadata.namespace+'-'+conf.name ]">
								...
								<td class="actions">
									<router-link :to="'/configurations/connectionpooling/'+conf.data.metadata.namespace+'/'+conf.name" title="Configuration Details">
