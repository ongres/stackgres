					...
					<template v-for="conf in config" v-if="(conf.data.metadata.namespace == $route.params.namespace)">
						<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpoolconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
							...
							<td class="actions">
								...
								<a v-if="iCan('delete','sgpoolconfigs',$route.params.namespace) && !conf.data.status.clusters.length" v-on:click="deleteCRD('sgpoolconfig',$route.params.namespace, conf.name)" class="delete" title="Delete Configuration">
