			...
			<ul class="breadcrumbs">
				...
				<li>
					...
					<template v-if="$route.params.hasOwnProperty('name')">
						<router-link :to="'/configurations/connectionpooling/'+$route.params.namespace" title="Postgres">SGPoolingConfigList</router-link>
					</template>
					<template v-else>
						SGPoolingConfigList
					</template>
				...
				<div>
					<template v-if="$route.params.hasOwnProperty('name')">
						<template v-for="conf in config" v-if="conf.name == $route.params.name">
							<router-link v-if="iCan('patch','sgpoolconfigs',$route.params.namespace)" :to="'/crd/edit/connectionpooling/'+$route.params.namespace+'/'+conf.name" title="Edit Configuration">
								Edit Configuration
							</router-link>
							<a v-if="iCan('create','sgpoolconfigs',$route.params.namespace)" v-on:click="cloneCRD('SGPoolingConfig', $route.params.namespace, conf.name)" title="Clone Configuration">
								Clone Configuration
							</a>
					...
					<template v-else>
						<router-link v-if="iCan('create','sgpoolconfigs',$route.params.namespace)" :to="'/crd/create/connectionpooling/'+$route.params.namespace" class="add">Add New</router-link>
					</template>
				</div>	
			</div>	
		</header>
						...
						<template v-if="!config.length">
							<tr class="no-results">
								<td colspan="3" v-if="iCan('create','sgpoolconfigs',$route.params.namespace)">
									No configurations have been found, would you like to <router-link :to="'/crd/create/connectionpooling/'+$route.params.namespace" title="Add New Connection Pooling Configuration">create a new one?</router-link>
								</td>
								<td v-else colspan="3">
						...
						<template v-for="(conf, index) in config">
							<template  v-if="(index >= pagination.start) && (index < pagination.end)">
								<tr class="base">
									<td class="hasTooltip">
										<span>
											<router-link :to="'/configurations/connectionpooling/' + $route.params.namespace + '/' + conf.name" class="noColor">
												{{ conf.name }}
											</router-link>
										</span>
									</td>
									...
									<td class="actions">
										<router-link :to="'/configurations/connectionpooling/' + $route.params.namespace + '/' + conf.name" target="_blank" class="newTab">
									...
									<td class="usedOn">
										<ul>
											<li v-for="cluster in conf.data.status.clusters">
												<router-link :to="'/cluster/status/'+$route.params.namespace+'/'+cluster" title="Cluster Details">
													{{ cluster }}
