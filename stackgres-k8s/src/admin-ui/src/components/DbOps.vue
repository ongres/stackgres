        ...
        <div class="content">
            ...
            <template v-else>
                ...
                <template v-for="(op, index) in dbOps" v-if="op.name == $route.params.name">
                    <template v-if="(op.data.spec.op == 'minorVersionUpgrade')">
                        <div class="clusterStatus flex flex-50" v-for="status in op.data.status.conditions" v-if="status.status == 'True'">
                            <template v-for="cluster in clusters" v-if="cluster.name == op.data.spec.sgCluster">
                                <div class="pods" v-if="cluster.status.podsReady > 0">
                                    <template v-for="(pod, index) in cluster.status.pods">
                                        <h3 class="header xsPad" v-if="pod.role == 'primary'" :set="primaryDisk = getBytes(pod.diskUsed)">
                                            Primary
                                        </h3>
                                        <h3 class="header xsPad" v-else-if="(index == 1)">
                                            Replicas
                                        </h3>
                                        <div class="pod" :class="pod.role">
                                            <div class="podInfo xsPad">
                                                ...
                                                <div class="podStatus floatRight">
                                            ...
                                            <div class="podStatus">
                                                ...
                                                <div class="podFooter xsPad">
                                                    <span class="connCount">
                                                        <strong class="blue">
                                                            {{ getPodConnections(cluster.name, pod.name, pod.connections * Math.floor(Math.random() * 10)) }}
                                                        </strong></br>
                                                        Connections
                                                    </span>
                                                ...
                                                <div class="connGraph chart-wrapper" title="Real Time Connections">
                                                    <apexchart 
                                                        ...
                                                        :series="[{
                                                            ...
                                                            data: podConnections[cluster.name][pod.name]
                                                        }]">
                                                    </apexchart>
                                                </div>
                                            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPodConnections(clusterName, podName, connections) {
                const vc = this;

                if(!vc.podConnections.hasOwnProperty(clusterName))
                    vc.podConnections[clusterName] = {}
                
                if(!vc.podConnections[clusterName].hasOwnProperty(podName))
                    vc.podConnections[clusterName][podName] = []

                vc.podConnections[clusterName][podName].push(connections)
