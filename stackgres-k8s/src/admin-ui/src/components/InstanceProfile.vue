			...
			<table id="profiles" class="profiles pgConfig resizable" v-columns-resizable>
				...
				<tbody>
					...
					<template v-for="(conf, index) in config">
						<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'profile-'+conf.data.metadata.namespace+'-'+conf.name, ( (index < pagination.start) || (index >= pagination.end) ? 'hide' : '' ) ]" :data-name="conf.name">
						...
						<tr :class="[$route.params.name == conf.name ? 'open details profile pgConfig' : 'details profile pgConfig',  ( (index < pagination.start) || (index >= pagination.end) ? 'hide' : '' ) ]">
							<td colspan="4">
...
<script>
    ...
    export default {
		...
		mounted: function() {
			...
			$(document).on('click', 'table.profiles tr.base > td:not(.actions)', function(){    
