			...
			<ul class="breadcrumbs">
				...
				<li>
					...
					<template v-if="$route.params.hasOwnProperty('name')">
						<router-link :to="'/profiles/'+$route.params.namespace" title="Namespace Overview">SGInstanceProfileList</router-link>
					</template>
					<template v-else>
				...
				<div>
					<template v-if="$route.params.hasOwnProperty('name')">
						<template v-for="conf in config" v-if="conf.name == $route.params.name">
							<router-link v-if="iCan('patch','sginstanceprofiles',$route.params.namespace)" :to="'/crd/edit/profile/'+$route.params.namespace+'/'+conf.name" title="Edit Profile">
								Edit Profile
							</router-link>
							<a v-if="iCan('create','sginstanceprofiles',$route.params.namespace)" v-on:click="cloneCRD('SGInstanceProfile', $route.params.namespace, conf.name)" title="Clone Profile">
								Clone Profile
							</a>
						...
						<template v-if="!config.length">
							<tr class="no-results">
								<td v-if="iCan('create','sginstanceprofiles',$route.params.namespace)" :colspan="5">
									No profiles have been found, would you like to <router-link :to="'/crd/create/profile/'+$route.params.namespace" title="Add New Instance Profile">create a new one?</router-link>
								</td>
								<td v-else colspan="5">
						...
						<template v-else>
							<template v-for="(conf, index) in config">
								<template  v-if="(index >= pagination.start) && (index < pagination.end)">
									<router-link :to="'/profiles/' + $route.params.namespace + '/' + conf.name" v-slot="{ navigate }" custom>
										<tr class="base">
											<td @click="navigate" class="hasTooltip configName">
												<span>{{ conf.name }}</span>
											</td>
											<td @click="navigate" class="memory fontZero">{{ conf.data.spec.memory }}</td>
											<td @click="navigate" class="cpu fontZero">{{ conf.data.spec.cpu }}</td>
											<td class="actions">
												<router-link v-if="iCan('patch','sginstanceprofiles',$route.params.namespace)" :to="'/crd/edit/profile/'+$route.params.namespace+'/'+conf.name" title="Edit Configuration">
