            ...
            <ul class="breadcrumbs">
                ...
                <li>
                    ...
                    <template v-if="$route.params.hasOwnProperty('name')">
						<router-link :to="'/' + $route.params.namespace + '/sgdistributedlogs'" title="Logs Servers">SGDistributedLogs</router-link>
					</template>
					<template v-else>
						SGDistributedLogs
					</template>
                ...
                <div>
					<template v-if="$route.params.hasOwnProperty('name')">
						<template v-for="cluster in clusters" v-if="cluster.name == $route.params.name">
                            ...
                            <a v-if="iCan('delete','sgdistributedlogs',$route.params.namespace)" @click="deleteCRD('sgdistributedlogs',$route.params.namespace, cluster.data.metadata.name, '/' + $route.params.namespace + '/sgdistributedlogs')" class="delete" title="Delete Configuration" :class="cluster.data.status.clusters.length ? 'disabled' : ''">
                                Delete Logs Cluster
                            </a>
                        ...
                        <template v-for="(cluster, index) in clusters">
                            <template  v-if="(index >= pagination.start) && (index < pagination.end)">
                                <tr class="base">
                                    ...
                                    <td class="actions">
                                        <router-link :to="'/' + $route.params.namespace + '/sgdistributedlog/' + cluster.name" target="_blank" class="newTab">
                                        ...
                                        <router-link v-if="iCan('patch','sgdistributedlogs',$route.params.namespace)" :to="'/' + $route.params.namespace + '/sgdistributedlog/' + cluster.data.metadata.name + '/edit'" title="Edit Configuration">
