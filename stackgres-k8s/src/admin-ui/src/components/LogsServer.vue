            ...
            <ul class="breadcrumbs">
                ...
                <li>
                    ...
                    <template v-if="$route.params.hasOwnProperty('name')">
						<router-link :to="'/logs/'+$route.params.namespace" title="Postgres">SGDistributedLogs</router-link>
					</template>
					<template v-else>
						SGDistributedLogs
					</template>
                ...
                <div>
					<template v-if="$route.params.hasOwnProperty('name')">
						<template v-for="cluster in clusters" v-if="cluster.name == $route.params.name">
							<router-link v-if="iCan('patch','sgdistributedlogs',$route.params.namespace)" :to="'/crd/edit/logs/'+$route.params.namespace+'/'+cluster.data.metadata.name" title="Edit Logs Cluster">
                               Edit Logs Cluster
                            </router-link>
                            <a v-if="iCan('create','sgdistributedlogs',$route.params.namespace)" v-on:click="cloneCRD('SGDistributedLogs', $route.params.namespace, cluster.data.metadata.name)" class="cloneCRD" title="Clone Logs">
                                Clone Logs Cluster
                            </a>
                    ...
                    <tbody>
                        ...
                        <template v-for="(cluster, index) in clusters">
                            <template  v-if="(index >= pagination.start) && (index < pagination.end)">
                                <router-link :to="'/logs/' + $route.params.namespace + '/' + cluster.name" v-slot="{ navigate }" custom>
                                    <tr class="base">
                                        <td @click="navigate" class="hasTooltip clusterName">
                                            <span>{{ cluster.name }}</span>
                                        </td>
                                        <td @click="navigate" class="volumeSize fontZero">
