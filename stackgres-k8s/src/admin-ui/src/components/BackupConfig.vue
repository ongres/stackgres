				...
				<tbody>
					<tr class="no-results">
						<td colspan="8" v-if="iCan('create','sgbackupconfigs',$route.params.namespace)">
							No configurations have been found, would you like to <router-link :to="'/crd/create/backupconfig/'+$route.params.namespace" title="Add New Backup Configuration">create a new one?</router-link>
						</td>
						<td v-else colspan="8">
...
<script>
    ...
    export default {
		...
		mounted: function() {
			...
			$(document).on('click', 'table.backupConfig tr.base > td:not(.actions)', function(){    
