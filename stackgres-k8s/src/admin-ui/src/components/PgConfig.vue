				...
				<tbody>
					...
					<template v-for="conf in config" v-if="(conf.data.metadata.namespace == $route.params.namespace)">
						<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpgconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
							...
							<td class="actions">
								<router-link v-if="iCan('patch','sgpgconfigs',$route.params.namespace) && !conf.data.status.clusters.length" :to="'/crd/edit/pgconfig/'+$route.params.namespace+'/'+conf.name" title="Edit Configuration">
								...
								<a v-if="iCan('delete','sgpgconfigs',$route.params.namespace) && !conf.data.status.clusters.length" v-on:click="deleteCRD('sgpgconfig',$route.params.namespace, conf.name, '/configurations/postgres/' + $route.params.namespace)" class="delete" title="Delete Configuration">
...
<script>
    ...
    export default {
		...
		mounted: function() {
			...
			$(document).on('click', 'table.pgConfig tr.base > td:not(.actions)', function(){    
