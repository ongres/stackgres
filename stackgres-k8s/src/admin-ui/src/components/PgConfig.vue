			...
			<table id="postgres" class="configurations pgConfig resizable" v-columns-resizable>
				...
				<tbody>
					...
					<template v-for="(conf, index) in config">
						<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpgconfig-'+conf.data.metadata.namespace+'-'+conf.name, ( (index < pagination.start) || (index >= pagination.end) ? 'hide' : '' ) ]" :data-name="conf.name">
