									...
									<template v-if="back.data.hasOwnProperty('status') && (back.data.status.process.status !== 'Pending')">
										<tr class="base">
											...
											<td class="actions">
												...
												<a v-if="iCan('delete','sgbackups',$route.params.namespace)"  @click="deleteCRD('sgbackups',$route.params.namespace, back.data.metadata.name)" class="delete" title="Delete Backup">
