						...
						<template v-for="conf in config" v-if="conf.data.metadata.namespace == currentNamespace">
							<tr :class="[ $route.params.name == conf.name ? 'open' : '', 'sgbackupconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" class="base" :data-name="conf.name">
								...
								<td class="actions">
									...
									<a v-if="iCan('delete','sgbackupconfigs',$route.params.namespace) && !conf.data.status.clusters.length" v-on:click="deleteCRD('sgbackupconfig',currentNamespace, conf.name)" class="delete" title="Delete Configuration">
