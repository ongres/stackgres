var LogsCluster = Vue.component("LogsCluster", {
	template: `
        <div id="logs-cluster">
            <header>
            ...
            <div class="content">
                <table id="logs" class="logsCluster pgConfig">
                    ...
                    <tbody>
                        ...
                        <template v-for="cluster in clusters" v-if="(cluster.metadata.namespace == $route.params.namespace)">
                            <tr class="base" :class="[ $route.params.name == cluster.metadata.name ? 'open' : '', 'logs-'+cluster.metadata.namespace+'-'+cluster.metadata.name ]" :data-name="cluster.metadata.name">
                                <td class="hasTooltip clusterName"><span>{{ cluster.metadata.name }}</span></td>
                                <td class="volumeSize fontZero">{{ cluster.spec.persistentVolume.size }}</td>
                                <td class="actions">
                                    <router-link v-if="iCan('patch','sgdistributedlogs',$route.params.namespace)" :to="'/admin/crd/edit/logs/'+$route.params.namespace+'/'+cluster.metadata.name" title="Edit Configuration">
                            ...
                            <tr :style="$route.params.name == cluster.metadata.name ? 'display: table-row' : ''" :class="$route.params.name == cluster.metadata.name ? 'open details logsCluster pgConfig' : 'details logsCluster pgConfig'">
                                <td colspan="3">
                                        ...
                                        <table>
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>{{ cluster.spec.persistentVolume.size }}</td>
                                                </tr>
                                                <template v-if="cluster.status.clusters.length">
                                                    <tr>
                                                        <td class="label">Used on</td>
                                                        <td class="usedOn">
                                                            <ul>
                                                                <li v-for="c in cluster.status.clusters">
                                                                    {{ c }}
