var CreateCluster = Vue.component("CreateCluster", {
    template: `
        <form id="create-cluster" class="noSubmit" v-if="loggedIn && isReady && !notFound">
            <!-- Vue reactivity hack -->
            ...
            if( (vm.$route.name === 'EditCluster') && !vm.editReady) {
                store.state.clusters.forEach(function( c ){
                    if( (c.data.metadata.name === vm.$route.params.name) && (c.data.metadata.namespace === vm.$route.params.namespace) ) {
                        ...
                        vm.nodeSelector = hasProp(c, 'data.spec.pods.scheduling.nodeSelector') ? vm.unparseProps(c.data.spec.pods.scheduling.nodeSelector, 'label') : [];
                        vm.tolerations = hasProp(c, 'data.spec.pods.scheduling.tolerations') ? c.data.spec.pods.scheduling.tolerations : [];
