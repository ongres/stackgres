		...
		<div id="pg-config">
			<header>
				...
				<div class="actions">
					...
					<div>
						<router-link to="/admin/crd/create/pgconfig/" class="add">Add New</router-link>
					</div>
				</div>	
			</header>
					...
					<tbody>
						...
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpgconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
								...
								<td class="actions">
									<router-link :to="'/admin/configurations/postgres/'+conf.data.metadata.namespace+'/'+conf.name" title="Configuration Details" class="no-color">
										<svg xmlns="http://www.w3.org/2000/svg" width="18.556" height="14.004" viewBox="0 0 18.556 14.004"><g transform="translate(0 -126.766)"><path d="M18.459,133.353c-.134-.269-3.359-6.587-9.18-6.587S.232,133.084.1,133.353a.93.93,0,0,0,0,.831c.135.269,3.36,6.586,9.18,6.586s9.046-6.317,9.18-6.586A.93.93,0,0,0,18.459,133.353Zm-9.18,5.558c-3.9,0-6.516-3.851-7.284-5.142.767-1.293,3.382-5.143,7.284-5.143s6.516,3.85,7.284,5.143C15.795,135.06,13.18,138.911,9.278,138.911Z" transform="translate(0 0)"/><path d="M9.751,130.857a3.206,3.206,0,1,0,3.207,3.207A3.21,3.21,0,0,0,9.751,130.857Z" transform="translate(-0.472 -0.295)"/></g></svg>
									</router-link>
									<router-link :to="'/admin/crd/edit/pgconfig/'+currentNamespace+'/'+conf.name" title="Edit Configuration">
							...
							<tr :style="$route.params.name == conf.name ? 'display: table-row' : ''" :class="$route.params.name == conf.name ? 'open details pgConfig' : 'details pgConfig'">
								<td colspan="4">
									...
									<div class="paramDetails">
										...
										<table>
											<tbody>
												<tr v-for="param in conf.data.status['postgresql.conf']">
													<td class="label">
														{{ param.parameter }}
													</td>
													<td class="paramValue">
