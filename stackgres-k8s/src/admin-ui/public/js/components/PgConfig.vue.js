					...
					<tbody>
						...
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpgconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
								...
								<td class="actions">
									<router-link v-if="iCan('patch','sgpgconfigs',$route.params.namespace) && !conf.data.status.clusters.length" :to="'/admin/crd/edit/pgconfig/'+currentNamespace+'/'+conf.name" title="Edit Configuration">
									...
									<a v-if="iCan('delete','sgpgconfigs',$route.params.namespace) && !conf.data.status.clusters.length" v-on:click="deleteCRD('sgpgconfig',currentNamespace, conf.name, '/admin/configurations/postgres/'+currentNamespace)" class="delete" title="Delete Configuration">
