var ClusterOverview = Vue.component("ClusterOverview", {
	template: `
		...
		<template v-for="namespace in namespaces" v-if="(namespace == $route.params.namespace)">
			<header>
				...
				<a class="documentation" href="https://stackgres.io/doc/latest/04-postgres-cluster-management/01-postgres-clusters/" target="_blank" title="SGCluster Documentation">SGCluster Documentation</a>
					<div>
						<router-link :to="'/admin/crd/create/cluster/'+$route.params.namespace" class="add">Add New</router-link>
					</div>	
				</div>	
			</header>
		...
		<template v-else-if="notFound">
			<!--<script>window.location.href = '/admin/not-found.html'</script>-->
		</template>
		</div>`,
	data: function() {
	...
	computed: {
		...
		notFound () {
			return !store.state.allNamespaces.includes(this.$route.params.namespace)
		}
	},
	methods: {
