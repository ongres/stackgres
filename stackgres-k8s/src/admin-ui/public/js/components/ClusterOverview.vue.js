		...
		<template v-for="namespace in namespaces" v-if="(namespace == $route.params.namespace)">
			<header>
				...
				<a class="documentation" href="https://stackgres.io/doc/latest/04-postgres-cluster-management/01-postgres-clusters/" target="_blank" title="SGCluster Documentation">SGCluster Documentation</a>
					<div>
						<router-link :to="'/admin/crd/create/cluster/'+$route.params.namespace" class="add" v-if="iCan('create','sgclusters',$route.params.namespace)">Add New</router-link>
					</div>	
				</div>	
			</header>
				...
				<table class="clusterOverview" v-if="iCan('list','sgclusters',$route.params.namespace)">
					<thead>
						<th>
							...
							<span class="helpTooltip"  @mouseover="helpTooltip( 'SGCluster', 'metadata.name')"></span>
						</th>
						...
						<th>
							<span>CPU</span>
							<span class="helpTooltip" @mouseover="helpTooltip( 'SGInstanceProfile', 'spec.cpu')"></span>
						</th>

						<th>
							<span>Memory</span>
							<span class="helpTooltip" @mouseover="helpTooltip( 'SGInstanceProfile', 'spec.memory')"></span>
						</th>
						...
						<th>
							<span>Health</span>
							<span class="helpTooltip" @mouseover="helpTooltip( 'SGCluster', 'status.pods.health')"></span>
						</th>
					...
					<tbody>
						...
						<template v-for="cluster in clusters">
							<tr v-if="cluster.data.metadata.namespace == currentNamespace" :class="'sgcluster-'+cluster.data.metadata.namespace+'-'+cluster.name" >
								<td class="clusterName">
									<router-link :to="'/admin/cluster/status/'+currentNamespace+'/'+cluster.name" title="Cluster Status" data-active=".set.clu" class="no-color">
										{{ cluster.name }}
									</router-link>
								</td>
								<td class="instances">
