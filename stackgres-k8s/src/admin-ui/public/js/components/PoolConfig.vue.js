					...
					<tbody>
						...
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'sgpoolconfig-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
								...
								<td class="actions">
									<router-link v-if="iCan('patch','sgpoolconfigs',$route.params.namespace)" :to="'/admin/crd/edit/connectionpooling/'+currentNamespace+'/'+conf.name" title="Edit Configuration">
									...
									<a v-if="iCan('delete','sgpoolconfigs',$route.params.namespace)" v-on:click="deleteCRD('sgpoolconfig',currentNamespace, conf.name)" class="delete" title="Delete Configuration">
