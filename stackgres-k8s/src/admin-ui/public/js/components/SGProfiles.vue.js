				...
				<table id="profiles" class="profiles pgConfig">
					<thead class="sort">
						<th @click="sort('data.metadata.name')" class="sorted desc name">
							<span>
								Name
								<span class="helpTooltip" @mouseover="helpTooltip( 'SGInstanceProfile', 'metadata.name')"></span>
							</span>
						</th>
					...
					<tbody>
						...
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'profile-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
								...
								<td class="actions">
									<router-link v-if="iCan('patch','sginstanceprofiles',$route.params.namespace)" :to="'/admin/crd/edit/profile/'+currentNamespace+'/'+conf.name" title="Edit Configuration">
									...
									<a v-if="iCan('delete','sginstanceprofiles',$route.params.namespace)" v-on:click="deleteCRD('sginstanceprofile',currentNamespace, conf.name)" class="delete" title="Delete Configuration">
								...
								<td colspan="4">
									<div class="configurationDetails">
										...
										<table>
											<tbody>
												<tr>
													<td class="label">RAM <span class="helpTooltip" @mouseover="helpTooltip( 'SGInstanceProfile', 'spec.memory')"></span></td>
													<td>{{ conf.data.spec.memory }}</td>
												</tr>
												<tr>
													<td class="label">CPU <span class="helpTooltip" @mouseover="helpTooltip( 'SGInstanceProfile', 'spec.cpu')"></span></td>
