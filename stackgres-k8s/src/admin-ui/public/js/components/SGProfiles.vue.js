	...
	template: `
		<div id="sg-profile" v-if="loggedIn && isReady && !notFound">
			<header>
				...
				<div class="actions">
					<a class="documentation" href="https://stackgres.io/doc/0.9/reference/crd/sginstanceprofile/" target="_blank" title="SGInstanceProfile Documentation">SGInstanceProfile Documentation</a>
					<div>
						...
						<template v-for="conf in config" v-if="(conf.data.metadata.namespace == currentNamespace)">
							<tr class="base" :class="[ $route.params.name == conf.name ? 'open' : '', 'profile-'+conf.data.metadata.namespace+'-'+conf.name ]" :data-name="conf.name">
								...
								<td class="actions">
									...
									<a v-if="iCan('delete','sginstanceprofiles',$route.params.namespace) && !conf.data.status.clusters.length" v-on:click="deleteCRD('sginstanceprofile',currentNamespace, conf.name, '/admin/profiles/'+currentNamespace)" class="delete" title="Delete Configuration">
