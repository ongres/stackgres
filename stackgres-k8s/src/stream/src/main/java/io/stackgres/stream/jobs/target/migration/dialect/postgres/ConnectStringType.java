...
/**
 * An implementation of {@link JdbcType} that supports {@code STRING} connect
 * schema types.
 *
...
public class ConnectStringType extends AbstractConnectSchemaType {
  ...
  public String getTypeName(Schema schema, boolean isKey) {
    // Some source data types emit a column size, and we need to be careful about
    // using the provided
    // size as it may be relative to the source data type; however due to the
    // handling code on the
    // source, it may be emitting the value as a STRING.
    //
    // An example of this is BYTES data types. If the binary.handling.mode is set to
    // anything but to
    // emit the raw bytes; the received value's schema type is STRING; however in
    // the case of a field
    // like VARBINARY(15), the size of 15 will be included in the propagated source
    // details and in
    // this case we should ignore it.
    //
    // To do this, getColumnSqlType will only return a non OTHER type code if the
    // column type details
    // are provided and can be mapped to a logical CHAR/NCHAR/VARCHAR/NVARCHAR type;
    // otherwise it
    // should be assumed that the size should be resolved using defaults rather than
    // what was passed
    ...
    if (Types.OTHER != resolvedJdbcType) {
      ...
      // MySQL will not emit a column size when propagation is enabled and CHARACTER
      // columns
      // are detected. This causes Hibernate to incorrectly report the field as
      // "char($l)"
      // because the size won't be substituted with 1 in this case, so this will
      // explicitly
      ...
      // PostgreSQL bpchar type with unlimited length is propagated with a length
      // using Integer.MAX_VALUE
      // Creating a type with such length in PostgreSQL is forbidden.
      ...
      // The column propagation details either don't exist or did not map to a logical
      // STRING type
      // In this case, we apply the size defaults (no size for non-keys and max-key
      // for keys).
  ...
  private boolean hasNationalizedCharacterSet(Schema schema) {
    // MySQL emits a __debezium.source.column.character_set property to pass the
    // character set name
    // downstream in the pipeline, which is useful for the sink connector to resolve
    // whether the
