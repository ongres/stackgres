...
until grep -q '^EXIT_CODE=' "$SHARED_PATH/vacuum.out" 2>/dev/null
do
  sleep 1
done
...
EXIT_CODE="$(grep '^EXIT_CODE=' "$SHARED_PATH/vacuum.out" | cut -d = -f 2)"
TIMED_OUT="$(grep '^TIMED_OUT=' "$SHARED_PATH/vacuum.out" | cut -d = -f 2)"
