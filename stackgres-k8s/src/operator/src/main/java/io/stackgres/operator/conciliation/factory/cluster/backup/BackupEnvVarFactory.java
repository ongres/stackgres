...
import io.stackgres.common.crd.storages.OpenPgpStorageEncryption;
...
public class BackupEnvVarFactory {
  ...
  private Map<String, Map<String, String>> getStorageSecretReferences(
    ...
    return streamStorageSecretReferences(storage)
        ...
        .toMap(
            t -> t.v1, t -> t.v2
                .grouped(tt -> tt.v1.getKey())
                .toMap(
                    tt -> tt.v1, tt -> tt.v2.findFirst().orElseThrow().v2));
  }
  ...
  public Seq<SecretKeySelector> streamStorageSecretReferences(BackupStorage storage) {
    return Seq.of(
            ...
            Optional.ofNullable(storage.getEncryption())
            .map(StorageEncryption::getSodium)
            .map(SodiumStorageEncryption::getKey),
            Optional.ofNullable(storage.getEncryption())
            .map(StorageEncryption::getOpenpgp)
            .map(OpenPgpStorageEncryption::getKey),
            Optional.ofNullable(storage.getEncryption())
            .map(StorageEncryption::getOpenpgp)
            .map(OpenPgpStorageEncryption::getKeyPassphrase))
  ...
  private ImmutableMap<String, String> getBackupSecrets(
    ...
    return Seq.of(
        ...
        .map(StorageEncryption::getSodium)
        .map(sodiumConf -> Seq.of(
            getSecretEntry("WALG_LIBSODIUM_KEY",
                sodiumConf.getKey(),
                secrets))),
        Optional.of(storage)
        .map(BackupStorage::getEncryption)
        .map(StorageEncryption::getOpenpgp)
        .map(openpgpConf -> Seq.of(
            getSecretEntry("WALG_PGP_KEY",
                openpgpConf.getKey(),
                secrets),
            getSecretEntry("WALG_PGP_KEY_PASSPHRASE",
                openpgpConf.getKeyPassphrase(),
                secrets))))
        .filter(Optional::isPresent)
  ...
  private Tuple2<String, String> getSecretEntry(
      String envvar,
      SecretKeySelector secretKeySelector,
