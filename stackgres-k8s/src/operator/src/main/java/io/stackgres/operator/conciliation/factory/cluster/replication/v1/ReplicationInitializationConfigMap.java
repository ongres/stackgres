...
package io.stackgres.operator.conciliation.factory.cluster.replication.v1;
...
public class ReplicationInitializationConfigMap extends AbstractBackupConfigMap
  ...
  private ConfigMap buildSource(StackGresClusterContext context) {
    ...
    data.put(
        ...
        ClusterPathV1.PG_REPLICATION_BASE_PATH.path());
    data.put(
        ClusterPathV1.PG_REPLICATION_INITIALIZATION_FAILED_BACKUP_PATH.name(),
        ClusterPathV1.PG_REPLICATION_INITIALIZATION_FAILED_BACKUP_PATH.path());
  ...
  protected String getAwsS3CompatibleCaCertificateFilePath(ClusterContext context) {
    return ClusterPathV1.REPLICATION_INITIALIZATION_SECRET_PATH.path(context)
