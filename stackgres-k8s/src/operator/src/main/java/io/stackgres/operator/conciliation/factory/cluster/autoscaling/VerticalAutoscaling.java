  ...
  public Stream<HasMetadata> generateResource(StackGresClusterContext context) {
    ...
    return Optional.of(cluster.getSpec())
        ...
        .flatMap(autoscaling -> Stream.of(
            ...
            .withMaxAllowed(Seq.<Map.Entry<String, String>>of()
                .append(Stream.of("cpu")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForPatroniCpu)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .append(Stream.of("memory")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForPatroniMemory)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)))
            .withMode("Auto")
            ...
            .withMaxAllowed(Seq.<Map.Entry<String, String>>of()
                .append(Stream.of("cpu")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForPgbouncerCpu)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .append(Stream.of("memory")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForPgbouncerMemory)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)))
            .withMode("Auto")
            ...
            .withMaxAllowed(Seq.<Map.Entry<String, String>>of()
                .append(Stream.of("cpu")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForEnvoyCpu)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .append(Stream.of("memory")
                    .flatMap(resource -> Optional.of(cluster.getSpec().getAutoscaling())
                        .map(StackGresClusterAutoscaling::getMaxAllowedForEnvoyMemory)
                        .map(value -> Map.entry(resource, value))
                        .stream()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue)))
            .withMode("Auto")
