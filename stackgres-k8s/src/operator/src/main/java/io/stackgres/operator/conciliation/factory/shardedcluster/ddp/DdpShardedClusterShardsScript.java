...
public class DdpShardedClusterShardsScript implements ResourceGenerator<StackGresShardedClusterContext> {

  final LabelFactoryForShardedCluster labelFactory;

  @Inject
  public DdpShardedClusterShardsScript(LabelFactoryForShardedCluster labelFactory) {
  ...
  public Stream<HasMetadata> generateResource(StackGresShardedClusterContext context) {
    return Seq.of(Boolean.TRUE)
        ...
        .<HasMetadata>map(ignore -> StackGresShardedClusterForDdpUtil
            .getShardsScript(context))
        .map(config -> {
          config.getMetadata().setLabels(labelFactory.shardsLabels(context.getSource()));
          return config;
        });
  }
