...
public class ShardingSphereShardedClusterComputeNode implements ResourceGenerator<StackGresShardedClusterContext> {

  final LabelFactoryForShardedCluster labelFactory;

  @Inject
  public ShardingSphereShardedClusterComputeNode(LabelFactoryForShardedCluster labelFactory) {
  ...
  public Stream<HasMetadata> generateResource(StackGresShardedClusterContext context) {
    return Seq.of(Boolean.TRUE)
        ...
        .<HasMetadata>map(ignore -> StackGresShardedClusterForShardingSphereUtil
            .getCoordinatorComputeNode(context,
                labelFactory.coordinatorLabelsWithoutUid(context.getSource())))
        .map(config -> {
