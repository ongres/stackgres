...
public class Fluentd implements ContainerResourceFactory<StackGresDistributedLogs,
  ...
  public Container getContainer(StackGresDistributedLogsGeneratorContext context) {
    return new ContainerBuilder()
        ...
        .withArgs(""
            ...
            + "    then\n"
            + "      cat << INNER_EOF\n"
            + "SELECT * FROM dblink('host=" + ClusterStatefulSetPath.PG_RUN_PATH.path()
              + " port=" + Envoy.PG_PORT + " user=postgres dbname=''${database}''',"
            + "  'SELECT NULL FROM set_chunk_time_interval("
              + "''log_postgres'', INTERVAL ''${retention}'') UNION ALL"
              + " SELECT NULL FROM set_chunk_time_interval("
              + "''log_patroni'', INTERVAL ''${retention}'')') AS (set_chunk_time_interval text);\n"
            + "INNER_EOF\n"
            + "    fi\n"
            + ")\n"
            + "EOF\n"
            ...
            + "      continue\n"
            + "    fi\n"
            + "    echo \"Perform retention of ${retention} for database ${database}\"\n"
            + "    cat << EOF | run_query\n"
            + "SELECT * FROM dblink('host=" + ClusterStatefulSetPath.PG_RUN_PATH.path()
              + " port=" + Envoy.PG_PORT + " user=postgres dbname=''${database}''',"
            + "    'SELECT drop_chunks("
              + "date_trunc(''${retention#* }'', now()) - INTERVAL ''${retention}'',"
              + " ''log_postgres'') UNION ALL SELECT drop_chunks("
              + "date_trunc(''${retention#* }'', now()) - INTERVAL ''${retention}'',"
              + " ''log_patroni'')') AS (drop_chunks text);\n"
            + "END\n"
            + "EOF\n"
            + "  done\n"
            + "}\n"
            ...
            + "while true\n"
            + "do\n"
            + "  if has_config_changed || [ -z \"$PID\" ] || [ ! -d \"/proc/$PID\" ]\n"
            + "  then\n"
  ...
  public Stream<HasMetadata> streamResources(StackGresDistributedLogsGeneratorContext context) {
    ...
    final String databaseList = distributedLogsContext.getConnectedClusters()
        .stream()
        .map(FluentdUtil::databaseNameAndOptions)
        .collect(Collectors.joining("\n"));
    final Map<String, String> data = ImmutableMap.of(
        "fluentd.conf", configFile,
        "databases", databaseList.isEmpty() ? databaseList : databaseList + '\n');
