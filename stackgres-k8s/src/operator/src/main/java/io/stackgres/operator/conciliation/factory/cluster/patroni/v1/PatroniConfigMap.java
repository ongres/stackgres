...
public class PatroniConfigMap implements VolumeFactory<StackGresClusterContext> {
  ...
  public @NotNull ConfigMap buildSource(StackGresClusterContext context) {
    ...
    data.put("PATRONI_CONFIG_FILE", ClusterPathV1.PATRONI_CONFIG_FILE_PATH.path());
    ...
    data.put("PATRONI_POSTGRESQL_DATA_DIR", ClusterPathV1.PG_DATA_PATH.path());
    data.put("PATRONI_POSTGRES_UNIX_SOCKET_DIRECTORY", ClusterPathV1.PG_RUN_PATH.path());
    ...
    if (Optional.ofNullable(cluster.getSpec().getDistributedLogs())
      ...
      data.put("PATRONI_LOG_DIR", ClusterPathV1.PG_LOG_PATH.path());
