  ...
  public void validate(T review) throws ValidationFailed {
    switch (review.getRequest().getOperation()) {
      ...
      case UPDATE: {
        ...
        List<Tuple2<String, Optional<String>>> requiredExtensions = Seq.seq(getPostgresExtensions(review))
            .flatMap(List::stream)
        ...
        List<Tuple2<String, Optional<String>>> toInstallExtensions = Seq.seq(getToInstallExtensions(review))
            .flatMap(List::stream)
            .map(extension -> Tuple.tuple(
                extension.getName(), Optional.ofNullable(extension.getVersion())))
            .collect(ImmutableList.toImmutableList());
