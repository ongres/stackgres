...
class ShardedClusterShardsPersistentVolumeSizeExpansionValidatorTest extends
  ...
  protected PersistentVolumeSizeExpansionValidator<StackGresShardedClusterReview,
    ...
    return new ShardedClusterShardsPersistentVolumeSizeExpansionValidator(
  ...
  protected StackGresCluster getCluster(StackGresShardedCluster resource) {
    return StackGresShardedClusterUtil.getCoordinatorCluster(resource);
  }

  @Override
  protected void setVolumeSize(StackGresShardedCluster cluster, String size) {
    cluster.getSpec().getShards().getPod().getPersistentVolume().setSize(size);
  }
  ...
  protected String getStorageClassName(StackGresShardedCluster cluster) {
    return cluster.getSpec().getShards().getPod().getPersistentVolume().getStorageClass();
  }
