...
class ShardsScriptsConfigMutatorTest {

  protected static final ObjectMapper JSON_MAPPER = new ObjectMapper();

  private ShardsScriptsConfigMutator mutator;

  @BeforeEach
  void setUp() throws Exception {
    mutator = new ShardsScriptsConfigMutator();
  }
  ...
  void createClusterWithouIds_shouldAddThem() {
    ...
    review.getRequest().getObject().getSpec().getShards().getManagedSql().getScripts().stream()
  ...
  void updateClusterAddingAnEntry_shouldSetId() {
    ...
    review.getRequest().getObject().getSpec().getShards().getManagedSql().getScripts()
        .add(1, new StackGresClusterManagedScriptEntry());

    StackGresShardedCluster expected = JsonUtil.copy(review.getRequest().getObject());
    expected.getSpec().getShards().getManagedSql().getScripts().get(1).setId(13);
