...
class BackupValidationResourceTest extends ValidationResourceTest<StackGresBackupReview> {

  @BeforeEach
  public void setUp() {
    final BackupValidationResource resource =
        new BackupValidationResource(pipeline);
    this.resource = resource;

    review = AdmissionReviewFixtures.backup().loadCreate().get();

    deleteReview = AdmissionReviewFixtures.backup().loadDelete().get();
  }
